# 👋 Hi, I'm Andreas — Homelab Infrastructure & DevOps Engineer

## 🔧 What I Do

I build and manage self-hosted infrastructure at home — Proxmox, Docker, Kubernetes, and automation tooling.

## 📊 My Homelab

| Component | Details |
|-----------|---------|
| **Hypervisor** | Proxmox VE 2-node cluster (pve + pve2) |
| **Network** | OPNsense, WireGuard VPN, AdGuard DNS |
| **Monitoring** | Prometheus, Grafana, Loki, CrowdSec |
| **Automation** | n8n, HomeAssistant, Hermes Agent |
| **Storage** | NFS, Ceph, automated backups |
| **Security** | CrowdSec IDS/IPS, firewall, 2FA everywhere |

## 🚀 My Tools & Scripts

| Repo | Description | Language |
|------|-------------|----------|
| [go-cli-tools](https://github.com/magos-cyber/go-cli-tools) | CLI tools for homelab automation (Proxmox, Docker, K8s, Helm, Vault, Consul) | Go |
| [homelab-scripts](https://github.com/magos-cyber/homelab-scripts) | Complete toolkit for homelabing | Shell |
| [python-scripts](https://github.com/magos-cyber/python-scripts) | Python automation scripts | Python |
| [docker-compose-stacks](https://github.com/magos-cyber/docker-compose-stacks) | Production-ready Docker Compose stacks | Docker |
| [proxmox-homelab-toolkit](https://github.com/magos-cyber/proxmox-homelab-toolkit) | Proxmox VE automation toolkit | Shell |
| [sysadmin-bash-toolkit](https://github.com/magos-cyber/sysadmin-bash-toolkit) | Sysadmin Bash toolkit | Shell |
| [homelab-master-portal](https://github.com/magos-cyber/homelab-master-portal) | Master portal for all homelab tools | HTML |

## 📦 Packages

- [go-cli-tools Docker image](https://github.com/magos-cyber/go-cli-tools/pkgs/container/go-cli-tools) — `docker pull ghcr.io/magos-cyber/go-cli-tools`

## 📋 Projects

- [Homelab Infrastructure Board](https://github.com/users/magos-cyber/projects/1) — Kanban board for tracking homelab tasks

## 🛠️ Tech Stack

```
Proxmox VE • Docker • Kubernetes • Helm • Vault • Consul
Go • Python • Bash • Terraform • Ansible
Prometheus • Grafana • Loki • CrowdSec
OPNsense • WireGuard • AdGuard
```

## 📫 Connect

- 🌐 [Homelab Portal](https://github.com/magos-cyber/homelab-master-portal)
- 💼 Building self-hosted infrastructure since 2024

---

*Built with ❤️ in Greece*
